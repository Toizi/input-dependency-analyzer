...
namespace {
	struct DumpHashPass : public FunctionPass {
		static char ID;
		DumpHashPass() : FunctionPass(ID) {}
		virtual bool runOnFunction(Function &F){
			bool didModify = false;
			bool firstInstruction = true;
			for (auto& B : F) {
				dbgs() << "FunctionName:"<<F.getName();
				//we wanna add the dump hash to the main function only
				for (auto& I : B) {
					//Print a function name add the beginning
					if(firstInstruction){
						IRBuilder <> builder(&I);
                        			builder.SetInsertPoint(&I);
						printArg(&B, &builder, F.getName().str());
...
char DumpHashPass::ID = 0;
static RegisterPass<DumpHashPass> X("dump-hash","adds a dump hash call at the end of the main function, useful for testing verifiers");
// Automatically enable the pass.
// http://adriansampson.net/blog/clangpass.html
static void registerDumpHashPass(const PassManagerBuilder &,
		legacy::PassManagerBase &PM) {
	//PM.add(new CutVerticesPass());
	PM.add(new DumpHashPass());
}
static RegisterStandardPasses
RegisterMyPass(PassManagerBuilder::EP_EarlyAsPossible,
		registerDumpHashPass);
